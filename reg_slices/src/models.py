...
class Slices3DRegModel(nn.Module):
    def __init__(self, img_size=128, n_slices=12, mode='train'):
        super().__init__()
        self.mode = mode
        self.slices_generator = UNet(n_channels=3)
        ...
        self.fc_p = nn.Linear(3, 128)
        self.fc_s = nn.Linear(992, 128)
    ...
    def forward(self, feed_dict):
        ...
        feat_list, slices_rec = self.slices_generator(img_input) # n_bs * n_slices, 3, w, h
        slices_rec_ = slices_rec.view(n_bs, self.n_slices, 3, n_w, n_h).view(n_bs, self.n_slices * 3, n_w, n_h)
        ...
        feat_local_aggregated = torch.cat(feat_interp, dim=2)
        feat_local_aggregated_ = feat_local_aggregated.view(n_bs, self.n_slices, n_qry, 992).permute(0, 2, 1, 3).reshape(n_bs * n_qry, self.n_slices, 992)
        feat_qry = self.fc_p(qry_rot)
        feat_slice = self.fc_s(feat_local_aggregated_) # n_bs * n_qry, n_slices, 128
        ...
        img_slices_ = feed_dict['img_slices'].view(n_bs, self.n_slices, 3, n_w, n_h).view(n_bs * self.n_slices, 3, n_w, n_h)
